-- Drop existing type if it exists (from previous attempt)
...
DROP POLICY IF EXISTS "Users can create proposals" ON public.relationships;
DROP POLICY IF EXISTS "Partners can update relationship status" ON public.relationships;
